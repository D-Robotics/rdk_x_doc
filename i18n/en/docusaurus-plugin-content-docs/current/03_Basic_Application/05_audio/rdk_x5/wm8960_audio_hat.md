...
```
From the above, and with reference to the [Onboard Earphone Audio Port](in_board_es8326.md#operation), you can confirm that sound card 0 corresponds to the `WM8960 Audio Driver HAT` node. The devices exist, with device numbers `0-0` and `0-1`. The actual devices to operate are `pcmC0D0p` and `pcmC0D1c`.
