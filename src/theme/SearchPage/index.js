/**
 * Official Algolia SearchPage with product × firmware-version facet filters.
 * Copied from @docusaurus/theme-search-algolia so /search uses the same
 * DocScope constraints as the navbar search box.
 */
...
function SearchPageContent() {
  ...
  const makeSearch = useEvent((page = 0) => {
    ...
    algoliaHelper.clearRefinements('doc_scope');
    if (product && version) {
      algoliaHelper.addFacetRefinement('doc_scope', `${product}::${version}`);
    }
  ...
  }, [searchQuery, docsSearchVersionsHelpers.searchVersions, makeSearch, product, version]);
  useEffect(() => {
