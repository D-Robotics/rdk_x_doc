...
在 X5 中，共有 CPU/GPU 3D/BPU/DDR 支持调频，CPU 支持调压
...
不同的 DDR 颗粒支持的频点可能不一致，通过以下命令可获取当前支持的频点
...
1.8G 超频时，CPU 工作电压为 1V，其余频点对应的工作电压为 800mV 目前 CPU 支持的 governor
...
CPU core 在不使用时，可以将其拔出，也可以进一步降低功耗，除了 core0 不支持 hotpulg，core1~core7 均支持 hotplug。
...
对于 BPU/GPU/ISP/VIDEO CODEC/JPEG/HIFI 都是有独立的电源控制，目前这些设备只有在使用时（驱动 open）上电，使用结束（驱动 close）就会下电。可尽量降低 X5 SOC 的整体功耗。
